    ...
    public void onMessageReceived(@NonNull RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            String message = String.format(getApplicationContext().getResources().getString(R.string.push_notification),
                    Objects.requireNonNull(remoteMessage.getData().get("oldStatus")).toLowerCase(),
                    Objects.requireNonNull(remoteMessage.getData().get("newStatus")).toLowerCase());
            ...
            Integer duration = readFromStorage(filePath, "duration.txt") != null ? Integer.parseInt(Objects.requireNonNull(readFromStorage(filePath, "duration.txt"))) : -1;

            Float latitude = readFromStorage(filePath, "latitude.txt") != null ? Float.parseFloat(Objects.requireNonNull(readFromStorage(filePath, "latitude.txt"))) : -1;
            Float longitude = readFromStorage(filePath, "longitude.txt") != null ? Float.parseFloat(Objects.requireNonNull(readFromStorage(filePath, "longitude.txt"))) : -1;
